...
class NeuralNetwork:
    ...
    def make_from_neuron_vector(self, neuron_vector, layer_sizes):
        ...
        for layer_index in range(len(layer_sizes)):
            ...
            for neuron_index in range(layer_sizes[layer_index]):
                neurons.append(neuron_vector[current_index])
                current_index += 1
            self.layers.append(NeuronLayer(neurons))

    def get_neuron_vector(self):
        neuron_vector = []
        for layer in self.layers:
            for neuron in layer.neurons:
                neuron_vector.append(neuron)
...
class TestMakeNeuralNetwork(unittest.TestCase):
    ...
    def testVector(self):
        ...
        vector = n.get_neuron_vector()
        self.assertEqual(len(vector), 6)

        self.assertEqual(vector[0], n.layers[0].neurons[0])
        self.assertEqual(vector[1], n.layers[0].neurons[1])
        self.assertEqual(vector[2], n.layers[1].neurons[0])
        self.assertEqual(vector[3], n.layers[1].neurons[1])
        self.assertEqual(vector[4], n.layers[1].neurons[2])
        self.assertEqual(vector[5], n.layers[2].neurons[0])

        n2 = NeuralNetwork()
        n2.make_from_neuron_vector(vector, [2, 3, 1])

        self.assertEqual(vector[0], n2.layers[0].neurons[0])
        self.assertEqual(vector[1], n2.layers[0].neurons[1])
        self.assertEqual(vector[2], n2.layers[1].neurons[0])
        self.assertEqual(vector[3], n2.layers[1].neurons[1])
        self.assertEqual(vector[4], n2.layers[1].neurons[2])
        self.assertEqual(vector[5], n2.layers[2].neurons[0])

        layer_sizes = n2.get_layer_sizes()

        self.assertEqual(len(layer_sizes), 3)
        self.assertEqual(layer_sizes[0], 2)
        self.assertEqual(layer_sizes[1], 3)
        self.assertEqual(layer_sizes[2], 1)
